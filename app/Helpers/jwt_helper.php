...
if (!function_exists('newTokenJWT')) {
    ...
    if (!function_exists('validateTokenFromRequest')) {

    function validateTokenFromRequest($request)
    {
        helper('jwt'); // Per si encara no s'ha carregat
        $cfgAPI = new \Config\APIJwt();

        try {
            $tokenData = getToken($cfgAPI->config(), $request);
            return $tokenData['data']; // Retornem només la part útil
        } catch (\Exception $e) {
            return false;
        }
    }
